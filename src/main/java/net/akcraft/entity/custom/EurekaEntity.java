...
public class EurekaEntity extends TameableEntity {
    public final AnimationState idleAnimationState = new AnimationState();
    private int idleAnimationTimeout = 0;

    private int abilityCooldown = 0;

    public EurekaEntity(EntityType<? extends TameableEntity> entityType, World world) {
        super(entityType, world);
    }

    public static DefaultAttributeContainer.Builder createEurekaAttributes() {
    ...
    private void applyPassiveEffect() {
        ...
        List<LivingEntity> nearbyEntities = this.getWorld().getEntitiesByClass(LivingEntity.class, box,
                entity -> !(entity instanceof HostileEntity) && entity != this);

        for (LivingEntity entity : nearbyEntities) {
            StatusEffectInstance current = entity.getStatusEffect(StatusEffects.REGENERATION);
    ...
    protected @Nullable SoundEvent getAmbientSound() {
        return ModSounds.EUREKA_IDLE;
    }
    @Override
    protected @Nullable SoundEvent getHurtSound(DamageSource source) {
        return ModSounds.EUREKA_HURT;
    }
